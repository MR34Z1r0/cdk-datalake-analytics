...
try:
    ...
    df_m_region = spark_controller.read_table(data_paths.BIG_BAGIC, "m_region", cod_pais=cod_pais, have_principal = True)
    df_m_subregion = spark_controller.read_table(data_paths.BIG_BAGIC, "m_subregion", cod_pais=cod_pais, have_principal = True)
    ...
    df_t_historico_pedido_ades_cabecera = spark_controller.read_table(data_paths.BIG_BAGIC, "t_documento_pedido_ades", cod_pais=cod_pais)    
...
try:
    ...
    spark_controller.write_table(df_t_pedido, data_paths.DOMINIO_ECONORED, target_table_name, partition_columns_array)
except Exception as e:
    logger.error(e) 
    raise 