...
try:
    ...
    m_linea =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_linea", cod_pais=cod_pais)
    m_familia =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_familia", cod_pais=cod_pais)
    m_subfamilia =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_subfamilia", cod_pais=cod_pais)
    m_marca =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_marca", cod_pais=cod_pais)
    m_presentacion =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_presentacion", cod_pais=cod_pais)
    m_formato =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_formato", cod_pais=cod_pais)
    m_sabor =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_sabor", cod_pais=cod_pais)
    m_categoria =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_categoria", cod_pais=cod_pais)
    m_tipo_envase =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_tipo_envase", cod_pais=cod_pais)
    #m_aroma =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_aroma", cod_pais=cod_pais) #Econored: doesnt have data up to 14-04-2025
    m_gasificado =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_gasificado", cod_pais=cod_pais)
    #m_unidad_negocio =  spark_controller.read_table(data_paths.BIG_BAGIC, "m_unidad_negocio", cod_pais=cod_pais) Econored: doesnt have data up to 14-04-2025
...
try:
    ...
    tmp_m_articulo = m_articulo.alias("ma") \
        ...
        .select(
            ...
            lit("000").cast(StringType()).alias("cod_aroma"), #m_aroma case
            lit("AROMA DEFAULT").cast(StringType()).alias("desc_aroma"), #m_aroma case
            ...
            lit("default_cod_unidad_negocio").cast(StringType()).alias("cod_unidad_negocio"),
            lit("default_desc_unidad_negocio").cast(StringType()).alias("desc_unidad_negocio"),
            when(
    ...
    spark_controller.upsert(tmp_m_articulo, data_paths.DOMINIO_ECONORED, target_table_name, id_columns, partition_columns_array)
