...
try: 
    ...
    df_m_sucursal = spark_controller.read_table(data_paths.DOMINIO_ECONORED, "m_sucursal", cod_pais=cod_pais) 
    df_m_pais = spark_controller.read_table(data_paths.DOMINIO_ECONORED, "m_pais", cod_pais=cod_pais) 
    df_m_compania = spark_controller.read_table(data_paths.DOMINIO_ECONORED, "m_compania", cod_pais=cod_pais) 
...
try:
    ...
    spark_controller.upsert(df_dim_sucursal, data_paths.COMERCIAL_ECONORED, target_table_name, id_columns, partition_columns_array)    
except Exception as e:
    logger.error(e)
    raise