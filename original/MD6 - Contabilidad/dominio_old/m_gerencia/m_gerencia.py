...
try:
    ...
    df_m_gerencia = create_df_union("m_gerencia", databases, S3_PATH_STG_BM)
    # Load Dominio
    df_m_gerencia_dom = read_table("m_gerencia", S3_PATH_DOM)
    ...
    # CREATE
    try:
        logger.info("Starting creation of tmp_dominio_m_gerencia")
        tmp_dominio_m_gerencia = (
            df_m_gerencia.alias("mg")
            .join(df_m_compania.alias("mc"), col("mc.cod_compania") == col("mg.cod_compania"), "inner")
            ...
            .select(
                ...
                col("mg.id_sucursal"),
                col("mg.id_gerencia"),
                col("mg.cod_ejercicio"),
                col("mg.cod_gerencia"),
                col("mg.cod_sucursal"),
                col("mg.desc_gerencia"),
                col("mg.estado"),
                col("mg.fecha_creacion"),
                col("mg.fecha_modificacion"),
            )
        )
        logger.info(f"tmp_dominio_m_gerencia count: {tmp_dominio_m_gerencia.count()}")
    ...
    try:
        ...
        conditions = (
            tmp_dominio_m_gerencia["id_gerencia"]
            == df_m_gerencia_dom["id_gerencia"]
        ) & (
            tmp_dominio_m_gerencia["id_pais"]
            == df_m_gerencia_dom["id_pais"]
        )

        different_df = tmp_dominio_m_gerencia.join(df_m_gerencia_dom, conditions, "left_anti")
        # logger.info(f"different_df count after left_anti join: {different_df.count()}")

        df_m_gerencia_dom = df_m_gerencia_dom.unionByName(different_df)
        # logger.info(f"df_m_gerencia_dom count after union: {df_m_gerencia_dom.count()}")
    ...
    try:
        ...
        df_m_gerencia_dom = (
            df_m_gerencia_dom.alias("a")
            .join(
                tmp_dominio_m_gerencia.alias("b"),
                (col("a.id_gerencia") == col("b.id_gerencia"))
                & (col("a.id_pais") == col("b.id_pais")),
                "inner",
            )
            .select(
                col("a.id_gerencia").alias("id_gerencia"),
                ...
                coalesce(col("b.desc_gerencia"), col("a.desc_gerencia")).alias("desc_gerencia"),
        ...
        logger.info(f"df_m_gerencia_dom count after UPDATE operation: {df_m_gerencia_dom.count()}")
    ...
    try:
        ...
        table_name = "m_gerencia"
        df = df_m_gerencia_dom
